...
namespace Gaten.Net.Extensions
{
    public static class ByteExtension
    {
        ...
        public static string UnsignedToSignedHexString(this byte[] bytes)
        {
            switch (bytes.Length)
            {
                case 1:
                    ...
                    return (int8 >= 0 ? "+" : "-") + System.Math.Abs(int8).ToHexString();

                case 2:
                    short int16 = BitConverter.ToInt16(bytes);
                    return (int16 >= 0 ? "+" : "-") + System.Math.Abs(int16).ToHexString();

                case 4:
                    int int32 = BitConverter.ToInt32(bytes);
                    return (int32 >= 0 ? "+" : "-") + System.Math.Abs(int32).ToHexString();
        ...
        /// <summary>
        /// byte[] 배열 직렬화
        /// Gaten.Net.Serialization.Serializer.Serialize()보다 5배정도 빠름
        /// </summary>
        /// <param name="b1"></param>
        /// <param name="b2"></param>
        /// <returns></returns>
        ...
        public static double SameDegree(this byte[] capture, byte[] target)
        {
            ...
            {
                diffSum += System.Math.Abs(capture[i] - target[i]);
            }
