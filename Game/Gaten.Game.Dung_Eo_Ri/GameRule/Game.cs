namespace Gaten.Game.Dung_Eo_Ri.GameRule
{
    public class Game
    {
        ...
        public Game()
        {
            ...
            {
                { "비누", new Mob("비누", ImageUtil.GetResourceImage("Mobs/BossMobs/비누.png"), bossMobLevel, new List<Equip>()) }
            };

            Dungeons = new Dictionary<string, Dungeon>
            {
                { "튜토리얼 던전", new Dungeon("튜토리얼 던전", minMobLevel, dungeonWidth, dungeonHeight, dungeonDepth, BossMobs["비누"]) }
            };
