    ...
    public partial class MainForm : Form
    {
		...
		private void button2_Click(object sender, EventArgs e)
		{
				...
				int.Parse(yrange.Text) <= 192)
			{
				...
				FileStream fs = new FileStream(ProgramNameD, FileMode.Open);
				...
				for (i = 1; sr.Peek() >= 0; i++)
				{
					...
					{
						xvalue[i] = int.Parse(textt[i].Substring(0, 1));
					...
					{
						xvalue[i] = int.Parse(textt[i].Substring(0, 2));
					...
					{
						xvalue[i] = int.Parse(textt[i].Substring(0, 3));
					...
					while (xvalue[i] + xpos <= 0 || xvalue[i] + xpos >= 512)
					{
						xpos = rnd.Next(0, int.Parse(xrange.Text)) - int.Parse(xrange.Text) / 2;
					}
					while (yvalue[i] + ypos <= 0 || yvalue[i] + ypos >= 384)
					{
					...
					printt[i] = textt[i].Substring(textlength[i]);
				}
				...
				MessageBox.Show("변환 완료.", "", MessageBoxButtons.OK, MessageBoxIcon.None);
			}
			else
			{
				xrange.Text = "";
				yrange.Text = "";
				MessageBox.Show("범위입력이 잘못되었습니다.", "오류", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}
