...
namespace Gaten.GameTool.NGD.NFDManager
{
    public class NFDFileWriter
    {
        ...
        {
            using(FileStream stream = new FileStream(nfdBasePath + "Dungeon.nfd", FileMode.OpenOrCreate))
            {
        ...
        {
            using (FileStream stream = new FileStream(nfdBasePath + "Monster.nfd", FileMode.OpenOrCreate))
            {
                using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8))
                {
        ...
        {
            using (FileStream stream = new FileStream(nfdBasePath + "Material.nfd", FileMode.OpenOrCreate))
            {
                using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8))
                {
        ...
        {
            using (FileStream stream = new FileStream(nfdBasePath + "Equipment.nfd", FileMode.OpenOrCreate))
            {
                using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8))
                {
        ...
        {
            using (FileStream stream = new FileStream(nfdBasePath + "Potion.nfd", FileMode.OpenOrCreate))
            {
                using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8))
                {
