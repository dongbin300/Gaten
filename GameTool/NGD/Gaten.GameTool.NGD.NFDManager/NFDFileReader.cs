...
namespace Gaten.GameTool.NGD.NFDManager
{
    public class NFDFileReader
    {
        ...
        {
            using FileStream stream = new(nfdBasePath + "Dungeon.nfd", FileMode.Open);
        ...
        {
            using FileStream stream = new(nfdBasePath + "Monster.nfd", FileMode.Open);
            using StreamReader reader = new(stream, Encoding.UTF8);
            string data = reader.ReadToEnd();
        ...
        {
            using FileStream stream = new(nfdBasePath + "Material.nfd", FileMode.Open);
            using StreamReader reader = new(stream, Encoding.UTF8);
            string data = reader.ReadToEnd();
        ...
        {
            using FileStream stream = new(nfdBasePath + "Equipment.nfd", FileMode.Open);
            using StreamReader reader = new(stream, Encoding.UTF8);
            string data = reader.ReadToEnd();
            List<string> equipmentString = data.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries).ToList();

            List<Item> equipments = new();
        ...
        {
            using FileStream stream = new(nfdBasePath + "Potion.nfd", FileMode.Open);
            using StreamReader reader = new(stream, Encoding.UTF8);
            string data = reader.ReadToEnd();
